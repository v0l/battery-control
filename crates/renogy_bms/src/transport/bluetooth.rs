//! BLE transport for Renogy BT-1/BT-2 modules: write `FFD1`, notify `FFF1`.
...
const WRITE_UUID: Uuid = uuid16(0xffd1);
const NOTIFY_UUID: Uuid = uuid16(0xfff1);
...
/// True if the advertised name looks like a Renogy battery / BT module.
pub fn name_is_renogy(name: Option<&str>) -> bool {
    name.map(|n| {
        let l = n.to_ascii_lowercase();
        l.starts_with("bt-th") || l.starts_with("rbt") || l.starts_with("bt-1") || l.starts_with("bt-2")
    })
    .unwrap_or(false)
}

/// Scan for Renogy batteries, filtered by the `BT-TH`/`RBT` name prefix.
pub async fn scan(secs: u64) -> Result<Vec<BtDevice>> {
    ...
    for p in peripherals {
        ...
        if name_is_renogy(name.as_deref()) {
            out.push(BtDevice {
                id: p.id().to_string(),
                name,
                rssi,
...
impl Transport for BluetoothTransport {
    async fn open(&mut self) -> Result<()> {
        ...
        let notify = chars
            ...
            .cloned()
            .ok_or_else(|| Error::Transport("notify characteristic fff1 not found".into()))?;
        let write = chars
            .iter()
            .find(|c| c.uuid == WRITE_UUID)
            .cloned()
            .ok_or_else(|| Error::Transport("write characteristic ffd1 not found".into()))?;
